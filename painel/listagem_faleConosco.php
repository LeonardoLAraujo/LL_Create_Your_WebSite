<?php
    ...
    if(isset($_POST['txt_busca_usuario']) != ''){
        $sql = mysqli_query($conecta,"select * from tb_faleconosco where nome like '{$_POST['txt_busca_usuario']}%' order by nome asc ");
    }else{
        $sql = mysqli_query($conecta,"select * from tb_faleconosco order by nome asc");
    }

    if(isset($_GET['apagar'])){
        $sql = mysqli_query($conecta,"delete from tb_faleconosco where nome=" .$_GET['apagar']);
...
<body>  
    ...
    <table class="tabela" border="1" align="center">
        ...
        <tr>
            ...
            <td><a href="listagem_faleConosco.php?apagar='<?php echo $linha['nome']; ?>'"><i class="bi bi-x"></i></a></td>
        </tr>
